...
namespace Task7
{
    class DynamicList
    {
        ...
        /// <summary>
        /// Sort stack contain only integer item
        /// </summary>
        public void SortInt()
        {
            ...
            {
                object minValue = this[0];
                int minIndex = 0;
                for (int j = 1; j < newCount; j++)
                {
                    //if (this[j].ToString().CompareTo(minValue.ToString()) < 0)
                    if ((int)this[j] < (int)minValue)
                    {
                        minValue = this[j];
                        minIndex = j;
                    }
                }
                this.RemoveAt(minIndex);
                this.Add(minValue);
                newCount--;
            }
        }
